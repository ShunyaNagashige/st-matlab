...
function ex6_5()
    ...
    for EbN0_dB = EbN0_dB_range
        % シミュレーション実行
        ber(k) = anti_sim(nDataBits, EbN0_dB);
    ...
    % 結果を格納する配列を確保する
    anti_ber_theory = zeros(1, max / 2 + 1);
    ...
    hold on;
    semilogy(EbN0_dB_range, anti_ber_theory, '--*r' , 'linewidth', 2); % 横軸t，縦軸xでプロットする
...
% ローカル関数 anti_sim の定義ここから
function ber = anti_sim(nDataBits, EbN0_dB)
    %%%%%%%%%%%%%%%%%%%%%%%%
    % anti 信号波形パラメータ
    s0 = -1;
    s1 = 1;
    ...
    realSymbol = real(reciveSymbol); % antiなので実部を抽出
