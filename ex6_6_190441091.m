...
% 【注意】関数名 ex6_6 は，mファイルのファイル名前と一致させること
function ex6_6()
    clc;
    clear all;
    ...
    for EbN0_dB = EbN0_dB_range
        % シミュレーション実行
        ber(k) = othogonal_sim(nDataBits, EbN0_dB);
    ...
    % 結果を格納する配列を確保する
    orth_ber_theory = zeros(1, max / 2 + 1);
    ...
    hold on;
    semilogy(EbN0_dB_range, orth_ber_theory, '--*r' , 'linewidth', 2); % 横軸t，縦軸xでプロットする
...
% ローカル関数 othogonal_sim の定義ここから
function ber = othogonal_sim(nDataBits, EbN0_dB)
    %%%%%%%%%%%%%%%%%%%%%%%%
    % 直交信号波形パラメータ
    s0 = 1;
    s1 = j;
    ...
    realSymbol = real(reciveSymbol); % 実部を抽出
    imagSymbol = imag(reciveSymbol); % 虚部を抽出
    demodBit = zeros(1, nDataBits); % 受信ビット格納用の配列確保(MATLAB tips)
    for k=1:nDataBits
        if (realSymbol(k) > imagSymbol(k))
            demodBit(k) = 0;
        else
            demodBit(k) = 1;
        end
    ...
    % (おまけ)上記for文をMATLAB風に書くと以下
    % demodBit(realSymbol > imagSymbol) = 0;
    % demodBit(realSymbol <= imagSymbol) = 1;
    % おまけ ここまで
    %
    % 受信器ここまで
    %%%%%%%%%%%%%%%%
